...
export class CreateScanUseCase implements UseCase<CreateScanDTO, Promise<Response>> {
  ...
  public async execute(dto: CreateScanDTO): Promise<Response> {
    ...
    try {
      ...
      const usedSettings = ReportSettings.create({ diameterFactor, volumeFactor });
      if (usedSettings.isFailure) {
        return left(Result.fail<any>(usedSettings.getErrorValue()));
      }
      ...
      const aggregateRootResult = Result.combine([usedSettings, scanOrError]);

      if (aggregateRootResult.isFailure) {
        return left(Result.fail<any>(aggregateRootResult.getErrorValue()));
      }

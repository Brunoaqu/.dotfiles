...
export class CreateScanUseCase implements UseCase<CreateScanDTO, Promise<Response>> {
  ...
  public async execute(dto: CreateScanDTO): Promise<Response> {
    ...
    try {
      ...
      const diameterFactorOrError = DiameterFactor.create({ value: dto.usedSettings.diameterFactor });
      const volumeFactorOrError = VolumeFactor.create({ value: dto.usedSettings.volumeFactor });
      ...
      if (valueObjectResult.isFailure) {
        return left(Result.fail<any>(valueObjectResult.getErrorValue()));
      }
      ...
      const usedSettingsOrError = ReportSettings.create({ diameterFactor, volumeFactor });
      if (usedSettingsOrError.isFailure) {
        return left(Result.fail<any>(usedSettingsOrError.getErrorValue()));
      }

      const usedSettings: ReportSettings = usedSettingsOrError.getValue();
      ...
      if (scanOrError.isFailure) {
        return left(Result.fail<any>(scanOrError.getErrorValue()));
      }

      scan = scanOrError.getValue();
      ...
      scan = await this.scanRepo.getById(scan.reportId);

      return right(Result.ok<Scan>(scan)) as Response;
    } catch (err) {

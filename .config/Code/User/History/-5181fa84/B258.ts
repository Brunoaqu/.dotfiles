...
import { isAliveCheck } from '../database/knex/config/config';

// Only start when databases are connected
while (isAliveCheck() === false) {
  console.log(isAliveCheck());
}
...
app.listen(process.env.API_PORT || 5000, () => {
  log.info(
    { context: 'HttpServer' },
    `HTTP server listening at http://localhost:${process.env.API_PORT}`
  );
});

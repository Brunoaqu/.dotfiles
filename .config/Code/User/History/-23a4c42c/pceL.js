class KnexReportRepository {
  ...
  async get(filters) {
    ...
    if (!!filters.specie) query.whereRaw("Scans.specieFound LIKE '??'", 'like', `%${filters.specie}%`);
    if (!!filters.enterprise_name) query.where('ua.enterprise_name', 'like', `%${filters.enterprise_name}%`);

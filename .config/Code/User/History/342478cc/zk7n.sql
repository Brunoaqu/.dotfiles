...
FROM Reports
    ...
    INNER JOIN (
        SELECT reportId, GROUP_CONCAT((height + width) / 2 SEPARATOR ';') AS diameter FROM WoodLogs GROUP BY reportId
    ) AS WoodLogsProcessed ON Reports.reportId = WoodLogsProcessed.reportId
WHERE reportTypeId = 1 AND counter >= @counterMin AND counter <= @counterMax

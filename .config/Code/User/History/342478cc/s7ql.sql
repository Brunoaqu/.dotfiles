SELECT Reports.reportId AS ID,
    ...
    Scans.area AS Area,
    Scans.avgFrameVolume AS VolumeMedioDoQuadro INTO OUTFILE '/export_data/RelaroriosDieres-17-11-2023.csv' FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '"' LINES TERMINATED BY '\n'
FROM Reports
...
SELECT WoodLogsAux.reportId AS ID, Reports.counter AS Contador,
    WoodLogsAux.Diametro AS Diametro INTO OUTFILE '/export_data/RelaroriosDieresDiametros-17-11-2023.csv' FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '"' LINES TERMINATED BY '\n'
FROM WoodLogsAux

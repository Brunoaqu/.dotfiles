...
function M.init()
    ...
    -- Tree
    keymap(n_v, 'gc', function()
        require('Brunoaqu.keymaps.utils').cwd_current_buffer()
    end, default_settings)
    keymap(n_v, '<Leader>f', function()
        require('Brunoaqu.keymaps.utils').toggle_tree()
    end, default_settings)

    -- Cokeline
    keymap(n, 'Q', function()
        require('Brunoaqu.keymaps.utils').delete_buffer()
    end, default_settings)
    ...
    keymap(ex_t, '<C-s>', function()
        require('Brunoaqu.keymaps.utils').save_file()
    end, default_settings)
    ...
    keymap(n, '<leader>v', function()
        require('Brunoaqu.keymaps.utils').toggle_diffview()
    end)
...
function M.lspsaga()
    ...
    keymap(n, '[e', function()
        require('Brunoaqu.keymaps.utils').prev_diag()
    end, default_settings)
    keymap(n, ']e', function()
        require('Brunoaqu.keymaps.utils').next_diag()
    end, default_settings)
    keymap(n, '[E', function()
        require('Brunoaqu.keymaps.utils').prev_error()
    end, default_settings)
    keymap(n, ']E', function()
        require('Brunoaqu.keymaps.utils').next_error()
    end, default_settings)
...
function M.debugger()
    ...
    keymap(n, '<leader>s', function()
        require('Brunoaqu.keymaps.utils').dap_float_scope()
    end, default_settings)
    keymap(n, '<F1>', function()
        require('Brunoaqu.keymaps.utils').dap_toggle_ui()
    end, default_settings)
